...
public class RatingsService {
	...
	/**
	 * TODO: publisher not used.Stream.Collectors not used..
	 * 
	 */
	...
	public void submitRatings(Rating rating) {
		...
		Doctor doctorToBeModified = doctorRepository.findById(rating.getDoctorId()).get();
		Double newRating = ratingsRepository.findByDoctorId(rating.getDoctorId())
				.stream()
				.mapToDouble(r -> r.getRating())
				.average()
				.orElse(0);

		doctorToBeModified.setRating(newRating);
		doctorRepository.save(doctorToBeModified);
	}
